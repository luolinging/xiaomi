...
export default {
	data () {
		return {
			hotStatus: true, /* 搜索框中用以判断是否获取焦点 */
			headerStatus: false, /* 判断nav中的链接被鼠标经过时显示子选项数据 */
			tids: '',
			currentPhones: this.xiaomi, /* 临时对象 --用以指代，鼠标所处的位置是哪一项此对象中包含的就是哪一项的数据 ，初始值为第一项*/
			hotItems: ['红米pro', '小米笔记本air'], /* 搜索框中未获取焦点时默认显示的值 */
			xiaomi: [
	...
	methods: {
		/* 获取焦点时触发的事件 */
		evtIptFocus () {
			$('.header-search').addClass('search-active')/*  jQuery的写法---添加css中的定义的search-active类*/
			$('.search-result').show()
			this.hotStatus = false
		},
		/* 焦点离开时触发的事件 */
		evtIptBlur () {
			$('.header-search').removeClass('search-active')/*  jQuery的写法---删除css中的定义的search-active类*/
			$('.search-result').hide()
			this.hotStatus = true
		},
...
.header-search {
	...
	/* transition：css3的新属性，显示数据的时间将被延长 
	 * fadeOut：淡出，点击搜索框下面的数据会淡出以显示
	 */
	.fadeOut-transition {
